    ...
    fileprivate func filter(forceShowAll addAll: Bool) {
        ...
        for i in 0 ..< filterDataSource.count {
            ...
            if !inlineMode {
                ...
                let titleFilterRange = (item.title.lowercased() as NSString).range(of: text!, options: comparisonOptions)
                let subtitleFilterRange = item.subtitle != nil ? (item.subtitle!.lowercased() as NSString).range(of: text!, options: comparisonOptions) : NSMakeRange(NSNotFound, 0)
