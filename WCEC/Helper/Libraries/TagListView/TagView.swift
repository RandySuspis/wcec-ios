...
open class TagView: UIView {
    ...
    var button = UIButton()
    var tagId: Int = -1
    var tagType: TypeSubCategory = .interests
...
//            titleEdgeInsets.bottom = paddingY
    ...
    public init(title: String, tagID: Int, type: TypeSubCategory) {
    ...
    override open var intrinsicContentSize: CGSize {
        ...
        size.width += 10//paddingX * 2
        if size.width < size.height {
            size.width = size.height
        }
        
        if enableRemoveButton {
            size.width += removeButton.frame.size.width
        }
        if size.width > UIScreen.main.bounds.width - 30 {
            if enableRemoveButton {
                size.width = UIScreen.main.bounds.width - removeButton.frame.size.width
            } else {
    ...
    open override func layoutSubviews() {
        ...
        if enableRemoveButton {
            ...
            label.frame = CGRect.init(x: 0, y: 0, width: intrinsicContentSize.width - removeButton.frame.size.width, height: intrinsicContentSize.height)
