...
open class TLPhotoCollectionViewCell: UICollectionViewCell {
    ...
    var configure = TLPhotosPickerConfigure() {
        didSet {
            self.selectedView?.setGradientColor(colors: [AppColor.colorWhite50().cgColor, AppColor.colorBlack50().cgColor])
        }
    }
    ...
    @objc open var selectedAsset: Bool = false {
        willSet(newValue) {
            ...
            self.selectedIcon?.isHidden = !newValue
            self.durationView?.backgroundColor = AppColor.colorBlack54()
            if !newValue {
