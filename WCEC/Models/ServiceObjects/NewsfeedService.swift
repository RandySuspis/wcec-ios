...
class NewsfeedService: APIServiceObject {
    ...
    func getListNewfeed(authorId: Int? = nil, trending: [SubCategoryModel], searchText: String, pager: Pager, complete: @escaping(Result<APIResponse<[NewfeedModel], Any>>) -> Void) {
        let request = APIRequestProvider.shareInstance.listNewfeed(authorId: authorId, trending: trending, searchText: searchText, pager: pager)
        self.serviceAgent.startRequest(request) { (data, meta, error) in
            if let err = error {
                complete(Result.failure(err))
            } else {
    ...
    func getDetailPost(_ id: String, complete: @escaping(Result<APIResponse<NewfeedModel, Any>>) -> Void) {
        let request = APIRequestProvider.shareInstance.newfeedDetail(id)
        self.serviceAgent.startRequest(request) { (data, meta, error) in
            if let err = error {
                complete(Result.failure(err))
            } else {
                if let json = data {
                    let newfeedDTO = NewfeedDTO(json)
                    let newfeedModel = NewfeedModel(newfeedDTO)
                    complete(Result.success(APIResponse(json, data: newfeedModel)))
                }
            }
        }
    }
    ...
    func sharePostVia(postId: String, complete: @escaping(Result<APIResponse<NewfeedModel, Any>>) -> Void ) {
        let request = APIRequestProvider.shareInstance.sharePostVia(postId: postId)
        self.serviceAgent.startRequest(request) { (data, meta, error) in
            if let err = error {
                complete(Result.failure(err))
            } else {
    ...
    func deleteComment(_ id: String, complete: @escaping(Result<APIResponse<Any, Any>>) -> Void ) {
        let request = APIRequestProvider.shareInstance.deleteComment(id)
        self.serviceAgent.startRequest(request) { (data, meta, error) in
            if let err = error {
                complete(Result.failure(err))
            } else {
    ...
    func createComment(_ postId: String, content: String, complete: @escaping(Result<APIResponse<CommentModel, Any>>) -> Void ) {
        let request = APIRequestProvider.shareInstance.createComment(postId, content: content)
        self.serviceAgent.startRequest(request) { (data, meta, error) in
            if let err = error {
                complete(Result.failure(err))
            } else {
                if let json = data {
                    let newsfeedDTO = CommentDTO(json)
                    let model = CommentModel(newsfeedDTO)
                    complete(Result.success(APIResponse(json, data: model)))
                }
            }
        }
